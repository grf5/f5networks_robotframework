...
*** Test Cases ***
Remove CIDR Entry from BIG-IP SSHd Allow ACL
    Generate Token
...
Apply new SSH Allow ACL
    list should contain value       ${initial_sshd_allow_acl}       ${NEW_CIDR_ENTRY}
    ${new_sshd_allow_acl}           set variable                    ${initial_sshd_allow_acl}
    remove values from list         ${new_sshd_allow_acl}           ${NEW_CIDR_ENTRY}
...
Verify SSH Allow ACL Configuration
    ...
    list should not contain value   ${sshd_allow_acl}     ${NEW_CIDR_ENTRY}
    [Teardown]                      Delete All Sessions
